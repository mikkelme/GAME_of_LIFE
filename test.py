...
class GAME_of_LIFE:
    ...
    def launch(self):
        ...
        # x_axis_length = ax_pos[1,0] - ax_pos[0,0]
        # y_axis_length = ax_pos[1,1] - ax_pos[0,1]
        # x_max = 0.775
        # y_max = 0.77

        right_goal = 1 - 2*axis_padding - 1.5*std_button_length
        right = 1 - 2*axis_padding - 1.5*std_button_length
        plt.subplots_adjust(left = 0, right = right_goal)
        ax_pos = self.ax.get_position()._points
        plt.subplots_adjust(left = max(axis_padding, 2*(right_goal-ax_pos[1,0])), right = right_goal)

        # print(f"right: {ax_pos[1,0]}, right_goal: {right_goal}, diff: {right_goal-ax_pos[1,0]}")

        # print(x_axis_length, y_axis_length)
        # right = 1
        # available_left = right - x_max
        # print(x_max + 2*axis_padding + 1.5*std_button_length)
        #
        # left = available_left
        # left = 0
        ...
        # # left = right - axis_padding - x_axis_length
        # tmp = 0.03
        # left = axis_padding + (right - x_axis_length - 3*axis_padding)
        # # print(right-x_axis_length-left)
        # # print(x_axis_length, 1 - 2*axis_padding - 1.5*std_button_length - x_axis_length)
        #
        # # if dim[0] == dim[1]:
        # if True:
        #     # plt.subplots_adjust(left = available_x_padding + axis_padding, right = 1 - 2*axis_padding - 1.5*std_button_length)
        #     plt.subplots_adjust(left = left, right = right)
        #     print(right)

        # print(1 - 1.25*std_button_length - axis_padding - 0.25*std_button_length + 1.5*std_button_length)
        # print(1-2*axis_padding-1.5*std_button_length)
        # plt.subplots_adjust(left = 0.5*available_x_padding + axis_padding , right = 1 - 2*axis_padding - 1.5*std_button_length  - 0.5*available_x_padding)
        ...
        # Add random button
        random_ax = plt.axes([      0.25*std_button_length + axis_padding,
                                    axis_padding,
                                    std_button_length,
                                    std_button_height ])
        ...
        sym_ax = plt.axes([random_ax.get_position()._points[1,0] + spacer,
                                    random_ax.get_position()._points[0,1],
                                    random_sumbit_length,
                                    std_button_height])
        self.sym_button = Button(sym_ax, 'SYM',color='lightgrey', hovercolor='lightgreen')



        # Add sumbit textbox for random button
        random_input_ax = plt.axes([sym_ax.get_position()._points[1,0] + spacer,
        ...
        # Create slider
        slider_ax = plt.axes([random_input_ax.get_position()._points[1,0] + random_sumbit_length,
                                    random_ax.get_position()._points[0,1],
                                    1 - random_input_ax.get_position()._points[1,0] - random_sumbit_length - 2*axis_padding - 1.5*std_button_length,
                                    0.03])
        ...
        self.slider.text = slider_ax.text(1, 1.5 , f"Speed: {self.slider.val}/5    (Pause: {self.pause:.2f})")


        # Add start/stop button
        start_stop_ax = plt.axes([  1 - 1.25*std_button_length - axis_padding,
                                    axis_padding,
                                    std_button_length,
                                    std_button_height])
